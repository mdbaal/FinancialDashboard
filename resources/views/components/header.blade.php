<header class="bg-blue-300 drop-shadow-md h-20">
    <div class="max-w-screen-xl mx-auto flex flex-row py-5 gap-2 justify-between">
        <img class="rounded drop-shadow logo" src="{{url('images/logo.jpg')}}" alt="logo">
        <a href="#" class="header-btn bg-blue-400 text-white p-2 rounded drop-shadow">Dashboard</a>
        <a href="#" class="header-btn bg-blue-400 text-white p-2 rounded drop-shadow">Accounts</a>
        <a href="#" class="header-btn bg-blue-400 text-white p-2 rounded drop-shadow">Piggybanks</a>
        <a href="#" class="header-btn bg-blue-400 text-white p-2 rounded drop-shadow">Accounts</a>
        <a href="#" class="header-btn bg-blue-400 text-white p-2 rounded drop-shadow">Import Account</a>
    </div>
</header>
